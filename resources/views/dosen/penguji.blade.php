                                 ...
                                 <div class="dropdown-menu dropdown-menu-end fs-6">
                                    ...
                                    @if ($item->jenis_ujian == 'skripsi')
                                          <a class="dropdown-item py-2 {{ !$item->sk_pembimbing ? 'disabled' : '' }}"
                                             href="{{ asset('storage/' . $item->jenis_ujian . '/'. $item->mahasiswa->nama . '/' . $item->sk_pembimbing) }}" download>
                                             SK Pembimbing
                                          </a>
                                          <a class="dropdown-item py-2 {{ !$item->sk_penguji ? 'disabled' : '' }}"
                                             href="{{ asset('storage/' . $item->jenis_ujian . '/'. $item->mahasiswa->nama . '/' . $item->sk_penguji) }}" download>
                                             SK Penguji
                                          </a>
                                          <a class="dropdown-item py-2 {{ !$item->ba ? 'disabled' : '' }}"
                                             href="{{ asset('storage/' . $item->jenis_ujian . '/'. $item->mahasiswa->nama . '/' . $item->ba) }}" download>
                                             Berita Acara
                                          </a>
                                          <a class="dropdown-item py-2 {{ !$item->undangan ? 'disabled' : '' }}"
                                             href="{{ asset('storage/' . $item->jenis_ujian . '/'. $item->mahasiswa->nama . '/' . $item->undangan) }}" download>
                                             Undangan Skripsi
                                          </a>
                                          <a class="dropdown-item py-2 {{ !$item->lembar_penilaian ? 'disabled' : '' }}"
                                             href="{{ asset('storage/' . $item->jenis_ujian . '/'. $item->mahasiswa->nama . '/' . $item->lembar_penilaian) }}" download>
                                             Lembar Penilaian
                                          </a>
                                    @endif
                                 </div>
